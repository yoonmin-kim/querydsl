...
public class QuerydslBasicTest {
    ...
    /**
     * and 조건만 단독으로 쓸 경우 , 로 구분지어서 이어나갈 수 있다
     */
    @Test
    void searchAndParam() {
        Member findMember = queryFactory
...
//                .fetchOne();
     ...
     * 외부조인 불가능 -> 다음에 설명할 조인 on을 사용하면 외부 조인 가능
     */
    @Test
    void theta_join() {
    ...
    /**
     * 회원 과 팀을 조인하면서, 팀 이름이 teamA인 팀만 조인, 회원은 모두 조회
...
//                .where(team.name.eq("teamA"))
    ...
    private List<Member> searchMember1(String usernameCond, Integer ageCond) {
        ...
        BooleanBuilder builder = new BooleanBuilder();//넘어오는 파라미터가 null이 절대 아닐걸 확신한다면
                                                      // 생성자 파라미터로 바로 조건을 넣어줘도 됨.
    ...
    private List<Member> searchMember2(String usernameCond, Integer ageCond) {
        return queryFactory
                .selectFrom(member)
                .where(usernameEq(usernameCond), ageEq(ageCond))// where(null, ageEq(..)) 일 경우 null은 무시가된다
